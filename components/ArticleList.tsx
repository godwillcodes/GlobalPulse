...
const ArticleList = ({ initialArticles, totalResults, currentPage, pageSize }: Props) => {
  ...
  const [visibleArticles, setVisibleArticles] = useState<Article[]>(initialArticles.slice(0, 3)); // Show initial 6 articles
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8 lg:mt-10">
          {visibleArticles.map((article, index) => (
            <article key={index} className="relative flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow-md dark:shadow-gray-700 overflow-hidden transition-transform transform hover:scale-105">
              <div className="relative h-48">
                <img 
                  src={article.urlToImage || PLACEHOLDER_IMAGE} 
                  alt={article.title} 
                  className="w-full h-full object-cover"
                  onError={(e: any) => e.target.src = PLACEHOLDER_IMAGE} // Fallback if image fails to load
                />
